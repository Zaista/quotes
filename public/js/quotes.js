...
let quote,
  ...
  facebookURL =
    'https://www.facebook.com/sharer/sharer.php?u=https://quotes.jovanilic.com/quotes.html',
  twitterURL =
    'https://twitter.com/intent/tweet?url=https://quotes.jovanilic.com/quotes.html';
    ...
    success: function (result) {
      ...
      if (result.error) {
        ...
        if (result.error === 'Invalid ID') {
          $('#solution').text(
            'No quotes found with id: ' + urlParams.get('quote')
          );
        } else if (result.error === 'No quotes returned') {
          $('#solution').text(
            'Wow! It appears that you have solved all the quotes there are. Well done.'
          );
        } else {
...
$(document).ready(function () {
  ...
  $('#quote-submit').submit(function () {
    // Validate a quote
    if (
      $('#quote-author').val().length < 2 ||
      $('#quote-text').val().length < 10
    ) {
      // validation failed
    ...
    $.post(register_endpoint, $(this).serialize(), function (data) {
      if (data.success) {
        ...
        showAlert(
          'login',
          'danger',
          'Email already registered, enter the password.'
        );
      }
    });
  ...
  $('#donate').click(function () {
    ...
    var win = window.open(
      'https://www.paypal.me/jovanilic/' + amount + currency,
      '_blank'
    );
    win.focus();
  });
...
function nextLetter() {
  ...
  while ($('.underscores:eq(' + activeLetter + ')').text() !== '_') {
    ...
    if (count <= activeLetter) activeLetter = 0;
...
function goBack() {
  ...
  if (activeLetter > 0) activeLetter--;
  else activeLetter = count - 1;
  highlightLetters(activeLetter);
}
...
function goForth() {
  ...
  if (activeLetter < count - 1) activeLetter++;
  else activeLetter = 0;
  highlightLetters(activeLetter);
}
