...
$(function () {
    ...
    if (!response.error) {
      ...
      response.forEach((item) => {
        ...
        if (last_year !== date.getFullYear()) {
          $('.timeline').append(
            '<div class="group"><span class="year">' +
              date.getFullYear() +
              '</span></div>'
          );
        }
        ...
        if (item.author) {
          ...
          content =
            'User <i>' +
            item.email +
            '</i> registered to the Game of Quotes system.';
        ...
        $(timeline_item)
          .find('.timeline_date')
          .addClass('date_' + timeline_color);
        $(timeline_item)
          .find('.timeline_quote')
          .css('borderColor', timeline_color);
        $(timeline_item)
          .find('.timeline_icon')
          .addClass('fa-' + timeline_icon);
      ...
      $('.timeline_link').click(function (e) {
        window.location.replace(
          'quotes.html?quote=' + $(e.target).siblings('#timeline_link').val()
        );
      });
    } else {
      $('.timeline').remove();
  ...
  $('a[href="#profile"]').on('shown.bs.tab', function () {
    if (!isMobile.matches) {
      $('#user-card .card-body').innerHeight(
        $('#password-card .card-body').outerHeight()
      );
    }
  });
  ...
  $('.nav-tabs li.tab').click(function () {
    window.history.pushState(
      'stateObject',
      'Title',
      'profile?' + $(this).find('a').text().trim().toLowerCase()
    );
  });
